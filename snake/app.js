let initialState = {
  ...
  function buildInitialState(){
    $('#heading').text('🐍  Welcome, to Snake 🍎 ').css('color', '#E63E61')
    gameScore = 0
    snake = {
        body: [ [10, 2], [10, 3], [10, 4], [10, 5] ],
        nextDirection: [0, 1]
  ...
  function renderState(){
    ...
    initialState.board.forEach(function(row, rowIndex){
        row.forEach(function(column, columnIndex){
            ...
            boardElement.append(segmentElement)
        })
    })
  }
  ...
  function displayHighscore(){
    highscore.forEach(function(score, scoreIndex){
        $(`#scores${scoreIndex + 1}`).text(`${score}`)
  ...
  function gameover(){
    ...
    snakeGame = false
    $('.snake').addClass('dead')
    $('#heading').text('GAME OVER 😞').css('color', '#E63E61')
  }
  //// NEED TO FINISH THIS TOMORROW 10/06
